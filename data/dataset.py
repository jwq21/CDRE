...
class BaseDataset(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, split_file=None):
        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(img_root, ann_root, split_file)
        
        examples = self.train_examples + self.val_examples + self.test_examples
        super(BaseDataset, self).__init__(examples, {'image': image_field, 'text': text_field})
    ...
    @classmethod
    def get_samples(cls, img_root, ann_root, split_file=None):
        full_dataset_path = os.path.join(ann_root, 'dataset.json')
        if not os.path.exists(full_dataset_path):
            raise FileNotFoundError(f"Annotation file not found: {full_dataset_path}")
        with open(full_dataset_path, 'r') as f:
            karpathy_data = json.load(f)['images']

        all_samples = []
        for img_data in karpathy_data:
            img_path = os.path.join(img_root, img_data['filename'])
            for sentence in img_data['sentences']:
                all_samples.append(Example.fromdict({'image': img_path, 'text': sentence['raw']}))
        
        total_descriptions = len(all_samples)


        if not split_file or not os.path.exists(split_file):
            raise FileNotFoundError(f"Split file not provided or not found: {split_file}. Please run split_dataset.py first.")
        ...
        if max_index >= total_descriptions:
            raise ValueError(f"Index error: the maximum index in the index file ({max_index}) exceeds the total number of descriptions in the dataset ({total_descriptions}).")
...
class Sydney(BaseDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, split_file=None):

        super(Sydney, self).__init__(image_field, text_field, img_root, ann_root, split_file)

class UCM(BaseDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, split_file=None):
        super(UCM, self).__init__(image_field, text_field, img_root, ann_root, split_file)

class RSICD(BaseDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, split_file=None):
        super(RSICD, self).__init__(image_field, text_field, img_root, ann_root, split_file)